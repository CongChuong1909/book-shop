...
function ListAuthor() {
    ...
    const addHandler = async (data) => {
        ...
        await fetch('http://localhost:5000/author/create', requestOptions)
    ...
    const updateHandler = async (data, id) => {
        ...
        console.log(requestOptions);
        await fetch(`http://localhost:5000/author/update/${id}`, requestOptions)
            .then(response => console.log(response))
            .catch(error => console.log(error))
    ...
    const handleDeleteAuthor = (id) =>{
        ...
        fetch(`http://localhost:5000/author/delete/${id}`, requestOptions)
            .then(response => console.log(response))
            .catch(error => console.log(error));
        
        var newListAuthor = listAuthor;
        var list = newListAuthor.filter(function(item) { return item.id != id; })
        setListAuthor(list);
    }
  ...
  return (
    <>
      <Container fluid>
        <Row>
          <Col md="12">
          <Button onClick={()=>setViewAddAuthor(true)}>Add new AuthorBook</Button>
            {viewAddAuthor && <AddAuthor onAdd= {addHandler}  onClose = {handleCloseAddForm} />}
            {viewUpdateAuthor &&  <UpdateAuthor  onUpdate = {updateHandler} onClose = {handleCloseUpdateForm}  value = {valueUpdateAuthor}/>}
              ...
              <Card.Body className="table-full-width table-responsive px-0">
                ...
                <Table className="table-hover table-striped">
                  ...
                  <tbody>
                        ...
                        {listAuthor && listAuthor.map((item)=>(
                             <tr key= {item.id}>
                                <td><img src={item.image} alt="" /></td>
                                <td >{item.name}</td>
                                <td >{item.description}</td>
                                <td><Button onClick={()=>handleDeleteAuthor(item.id)}>x</Button></td>
                                <td><Button onClick={()=>handleUpdateAuthor(item.id, item.name, item.image, item.description)}>Edit</Button></td>
                            </tr>
                        ))}
