...
function TableList() {
    ...
    const fetchListUser = useCallback(async () => {
            const response = await fetch(
                `http://localhost:5000/book?page=${pagination}&search=&type=&author=`,
            );
            if (!response.ok) {
    ...
    const addHandler = (data)=>{
        ...
        if(data.name== listBook.name)
        {
            ...
            fetch('http://localhost:5000/book/create', requestOptions)
    ...
    const updateHandler = async(data, id) =>{
        ...
        await fetch(`http://localhost:5000/book/update/${id}`, requestOptions)
            .then(response => {console.log(response); setReload(true);})
            .catch(error => console.log(error))
    ...
    const handleDeleteBook= (id)=>
    {
        ...
        fetch(`http://localhost:5000/book/delete/${id}`, requestOptions)
            .then(response => console.log(response))
            .catch(error => console.log(error));
        
            var newList = listBook;
            var list = newList.filter(function(item) { return item.id != id; })
            setListBook(list);
    }

    const handleUpdate = (id, name, image, price, des, qty, author, category, publisher )=>{
        setViewUpdate(true);
        setValueUpdate({id, name, image,price, des, qty, author, category, publisher});
    }
  ...
  return (
    <>
      <Container fluid>
        <Row>
          <Col md="12">
          {!viewPurchase && <Button onClick={()=>setViewAddBook(true)}>Add new book</Button>}
          {!viewPurchase && <Button onClick={()=>setViewPurchase(true)}>Purchase</Button>}
          {viewPurchase && <Button onClick = {()=>setViewPurchase(false)}>Book</Button>}

            {viewAddBook && <AddBook onAdd = {addHandler} onClose = {handleCloseAddForm} />}
            {viewUpdate &&  <UpdateBook onUpdate = {updateHandler} onClose = {handleCloseUpdateForm}  value = {valueUpdate}/>}

            {viewPurchase && <BookImportView/>}
            {!viewPurchase && <Card className="strpied-tabled-with-hover">
              <Card.Header>
              ...
              <Card.Body className="table-full-width table-responsive px-0">
                  ...
                  <tbody>
                    {listBook && listBook.map((item)=>(
                        <tr key = {item.id}>
                            ...
                            <td>{item.note}</td>
                            <td><Button onClick={()=>handleDeleteBook(item.id)}>x</Button></td>
                            <td><Button onClick={()=>handleUpdate(item.id, item.name, item.image, item.price, item.description, item.quatiry, item.author, item.category, item.publisher)}>Edit</Button></td>
                        </tr>
                    ))}
                ...
                <div style={{width: "100%", display:"flex", justifyContent:"center"}}><Pagination>{items}</Pagination></div>
              </Card.Body>
            </Card>
            }
          </Col>
