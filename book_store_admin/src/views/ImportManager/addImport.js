...
function AddImport(props) {
    ...
    const fetchListUser = useCallback(async () => {
        const response = await fetch(
            `http://localhost:5000/book?page=&search=&type=&author=`,
        );
        if (!response.ok) {
    ...
    const submitHandler = (e) =>{
        ...
        const newListBook = bookValue;
            ...
            const objIndex = newListBook.findIndex((item =>item.name == inputValue));
            if(objIndex != -1)
            {
                const data = {
                                        ...
                                        <div className="col-xl-12 col-lg-12 col-md-12">
                                            <div className="form-group mb-3">
                                                ...
                                                />
                                                    ...
                                                    <datalist id="data" >
                                                        {
                                                        bookValue.map((item, key) =>
                                                        <option key={key} value={item.name}>{item.author.name} </option>
                                                        )}
                                                    </datalist>
                                            </div>
                                            <div className="form-group mb-3">
