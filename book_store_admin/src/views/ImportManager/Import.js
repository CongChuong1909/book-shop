...
function Import(props) {
    ...
    const handleCreateNewImport = () =>{
        ...
        const newDataImport = {
            ...
            fetch('http://localhost:5000/import/create', requestOptions)
    ...
    return (
       <>
           <Button onClick = {handleView} onClose = {handleClose}>AddPurchase</Button>
        ...
        <Table className="table-hover table-striped">
                  ...
                  <tbody>
                    {listImportHasImageAndName && listImportHasImageAndName.map((item)=>{
                        let total = item.price* item.amount;
                        totalImport+= total;
                        return <tr key = {item.idBook}>
                                    <td>{item.name}</td>
                    ...
                    <tr>
                        ...
                        <td>
                            Total: <span style={{color:"red", fontSize:"20px", fontWeight:"bold"}}>{totalImport}</span>
                        </td>
                    </tr>
