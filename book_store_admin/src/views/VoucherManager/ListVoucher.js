...
function ListVoucher() {
    ...
    const addHandler = async (data) => {
        ...
        await fetch('http://localhost:5000/voucher/create', requestOptions)
    ...
    const updateHandler = async (data, id) => {
        ...
        console.log(requestOptions);
        await fetch(`http://localhost:5000/voucher/update/${id}`, requestOptions)
            .then(response => console.log(response))
            .catch(error => console.log(error))
    ...
    const handleDeleteVoucher = (id) =>{
        ...
        fetch(`http://localhost:5000/voucher/delete/${id}`, requestOptions)
            .then(response => console.log(response))
            .catch(error => console.log(error));
            var newListVoucher = listVoucher;
            var list = newListVoucher.filter(function(item) { return item.id != id; })
            setListVoucher(list);
    }
  ...
  return (
    <>
      <Container fluid>
        <Row>
          <Col md="12">
          <Button onClick={()=>setViewAddVoucher(true)}>Add new Publisher</Button>
            {viewAddVoucher && <AddVoucher onAdd = {addHandler} onClose = {handleCloseAddForm} />}
            {viewUpdate &&  <UpdateVoucher onUpdate = {updateHandler} onClose = {handleCloseUpdateForm}  value = {valueUpdate}/>}
                ...
                <Table className="table-hover table-striped">
                  <thead>
                    <tr>
                      ...
                      <th className="border-0">reduce price</th>
                  ...
                  <tbody>
                        ...
                        {listVoucher.map((item)=>(
                             <tr key={item.id}>
                                ...
                                <td  >{item.qty}</td>
                                <td  >{item.mile}</td>
                                <td><Button onClick={()=>handleDeleteVoucher(item.id)}>x</Button></td>
                                <td><Button onClick={()=>handleUpdateVoucher(item.id, item.name, item.reduce, item.qty, item.mile)}>Edit</Button></td>
                            </tr>
                        ))}
                  </tbody>
                </Table>
