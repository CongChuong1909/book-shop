...
function TableListCategory() {
    ...
    const addHandler = async (data) => {
        ...
        if(a==true)
        {
            ...
            await fetch('http://localhost:5000/type/create', requestOptions)
    ...
    const updateHandler = async (data, id) => {
        ...
        console.log(requestOptions);
        await fetch(`http://localhost:5000/category/update/${id}`, requestOptions)
            .then(response => console.log(response))
            .catch(error => console.log(error))
    ...
    const handleDeleteCate = (id) =>{
        ...
        fetch(`http://localhost:5000/type/delete/${id}`, requestOptions)
            .then(response => console.log(response))
            .catch(error => console.log(error));
        var newListCate = listCategory;
        var list = newListCate.filter(function(item) { return item.id != id; })
  ...
  return (
    <>
      <Container fluid>
        <Row>
          <Col md="12">
          <Button onClick={()=>setViewAddBook(true)}>Add new CategoryBook</Button>
            {viewAddBook && <AddCategory onAdd={addHandler} onClose = {handleCloseAddForm} />}
            {viewUpdateCategory &&  <UpdateCategory onUpdate = {updateHandler} onClose = {handleCloseUpdateForm}  value = {valueUpdateCategory}/>}
                  ...
                  <tbody>
                        ...
                        {listCategory.map((item)=>(
                             <tr>
                                ...
                                <td><Button onClick={()=>handleDeleteCate(item.id)}>x</Button></td>
                                <td><Button onClick={()=>handleUpdateCate(item.id, item.name)}>Edit</Button></td>
                            </tr>
                        ))}
