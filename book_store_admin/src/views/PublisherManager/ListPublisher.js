...
function ListPublisher() {
    ...
    const addHandler = async (data) => {
        ...
        await fetch('http://localhost:5000/publisher/create', requestOptions)
    ...
    const updateHandler = async (data, id) => {
        ...
        console.log(requestOptions);
        await fetch(`http://localhost:5000/publisher/update/${id}`, requestOptions)
            .then(response => console.log(response))
            .catch(error => console.log(error))
    ...
    const handleDeletePublisher = (id) =>{
        ...
        fetch(`http://localhost:5000/publisher/delete/${id}`, requestOptions)
            .then(response => console.log(response))
            .catch(error => console.log(error));

        var newListPublisher = listPublisher;
        var list = newListPublisher.filter(function(item) { return item.id != id; })
  ...
  return (
    <>
      <Container fluid>
        <Row>
          <Col md="12">
          <Button onClick={()=>setViewAddPublisher(true)}>Add new Publisher</Button>
            {viewAddPublisher && <AddPublisher onAdd = {addHandler} onClose = {handleCloseAddForm} />}
            {viewUpdatePublisher &&  <UpdatePublisher onUpdate = {updateHandler}  onClose = {handleCloseUpdateForm}  value = {valueUpdatePublisher}/>}
                ...
                <Table className="table-hover table-striped">
                  ...
                  <tbody>
                        ...
                        {listPublisher.map((item)=>(
                             <tr key={item.id}>
                                <td  >{item.name}</td>
                                <td><Button onClick={()=>handleDeletePublisher(item.id)}>x</Button></td>
                                <td><Button onClick={()=>handleUpdatePublisher(item.id, item.name)}>Edit</Button></td>
                            </tr>
                        ))}
                  </tbody>
                </Table>
